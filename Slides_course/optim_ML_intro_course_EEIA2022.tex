...
\end{itemize}
The code, the slides and the project statement are available at \url{https://github.com/rleriche/Optimization_AI_Cotonou_2022}
\end{frame}
...
$f(x) = \frac{1}{2} x^\top H x$ in $d=10$ dimensions, $H>0$, not aligned with the axes, condition number = 10.
\begin{center}
\mbox{
...
$\mathcal S$ is an hypercube of $\Rset[d]$, $\mathcal S = [LB,UB] \subset \Rset[d]$.
\vskip\baselineskip
...
\label{eq:sphereL1}
\end{equation}
First term: sphere function centered at $c$, $c_i = i~,~i=1,\ldots,d$. A simplistic model to the mean square error of a NN where $c$ minimizes the training error.\\
...
This phenomenon occurs more often when $d$ is large and is less obvious for $d=2$. \\
