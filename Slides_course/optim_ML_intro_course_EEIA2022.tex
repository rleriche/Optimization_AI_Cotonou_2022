...
\hess f(x) = 
\begin{bmatrix}
\frac{f(x)}{\partial x_1^2} & \frac{\partial^2 f(x)}{\partial x_1 \partial x_2} & \ldots & \frac{\partial^2 f(x)}{\partial x_1 \partial x_n}\\ 
...
\end{equation*}
a good approximation to what happen on any function when converging
\mbox{
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}
\frametitle{Towards ML: regularized quadratic function} 
...
\label{eq:sphereL1}
\end{equation}
First term: sphere function centered at $c$, $c_i = i~,~i=1,\ldots,n$. A simplistic model to the mean square error of a NN where $c$ minimizes the training error.\\
...
\frametitle{Exercise} 
\begin{enumerate}
\item Program the regularized sphere function (\ref{eq:sphereL1}). 
\item Minimize it with any version of the previous descent algorithm with a line search in 
dimension $n=10$ with $\mathcal S = [-5,5]^n$. 
...
\frametitle{Solution}
\begin{enumerate}
\item Cf. function \texttt{sphereL1} at the end of file \texttt{test\_functions}.
\item ~\\
{\scriptsize
...
As $\lambda$ increases, more $x_i$'s tend to 0. Because $c_i=i$, the components of lower rank are closer to 0, they are thus the first ones to be set to 0. \\
This phenomenon can be better understood by interpreting Problem ~(\ref{eq:sphereL1}) as a constrained optimization problem.
\begin{equation*}
\left\{
\begin{array}{l}
...
\end{equation*}
The first 2 terms are the regularized function (\ref{eq:sphereL1}), the last term doesn't depend on $x$.
Let's draw the sphere function and the constraint boundary $\lVert x \rVert_1$:
\begin{center}
 \includegraphics[height=5cm]{l1sphere_regularization-crop.pdf}
\end{center}
It is seen that the solution tends to be at a vertex of the feasible domain, where some of the $x$ components cancel out. 
This phenomenon occurs more often when $n$ is large and is less obvious for $n=2$. \\
If the $x$'s were a NN weights, setting $x_i$ to 0 is equivalent to cutting the corresponding link, thus reducing the NN capacity, thus making it less prone to overfitting. Finding the best value for $\lambda$ is something of an art.
\end{enumerate}
\end{frame}
...
\begin{equation*}
\begin{split}
& \min_{x \in [LB,UB] \subset \Rset[n]} f_\lambda(x) = f(x) + \lambda \lVert x \rVert_1 \\
& \text{ where, for classification, } \\
& f(x) = - \sum_{i=1}^N \{ t^i \log(y(e^i;x)) + (1-t^i) \log(1 - y(e^i;x))\} \\
& \text{ and for regression,} \\
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}%[allowframebreaks]
\frametitle{Exercise: regularization vs. training data size} 
Study the relationship between the number of training points and the best choice of the regularization parameter.
\vskip\baselineskip
To do this, use the file \texttt{main\_neural\_net} from \url{https://github.com/rleriche/Optimisation_AI_Cotonou2021}.
It contains the code for a 2 inputs NN in regression, \texttt{ntrain} is the number of training points, \texttt{lambda} the regularization parameter. \\
Try all the combinations between \texttt{ntrain}=5, 10 et 50, and $\lambda=0.01,~0.1$ et 1.
\vskip\baselineskip
What do you notice on the test error (\texttt{rmsetest}) ? How do you explain it ?
How does the training error evolve (\texttt{rmsetrain}) ?
Same question with the calculated solutions $x^\star$?
\end{frame}
...
\hline
\end{tabular}
\caption{Root Mean Square Error on the test set, \texttt{ntest=100}.
In red: smallest error per row (i.e., per training data set size).}
\end{table}
When \texttt{ntrain} is small, it is best to regularize a lot: $\lambda=1$ is the best setting with 5 training points.
Vice versa, a large training set (50 points) does not need much regularization. 
There is a trade-off between the NN flexibility  (that increases when $\lambda$ decreases) and the number of training points that constraint the NN: 
a flexible NN with little constraints may be wrong away from the data points.
\vskip\baselineskip

\texttt{rmsetrain} increases with $\lambda$ since the emphasis is put on the minimization of $\lVert x \rVert_1$ at the expense of data fitting. 
\vskip\baselineskip
When $\lambda$ is large, some weights of the NN cancel out. 
The number of null weights grows when the size of the training set diminishes.
\end{frame}
