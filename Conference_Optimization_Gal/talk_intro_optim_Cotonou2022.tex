...
\begin{frame}
\frametitle{Abstract} 
The study of optimization algorithms started at the end of World War II with a constantly growing interest, fueled by needs in engineering, computational simulation and Machine Learning.
In this talk, we look into the history of the new scientific objects that are the optimizers. 
The point-of-view will not be mathematical, but rather historical and philosophical.
The conditions for the emergence of optimizers correspond to a decomposition principle in science. 
Although optimizers are tools, they guide through a space of representations which fascinates because it resembles learning. 
Optimization problems are also utopias: rationality is bounded, they are over-simplifications of a human-based decision process and computational capacities are limited.  
The presentation finishes with recent challenges in optimization.
\end{frame}
